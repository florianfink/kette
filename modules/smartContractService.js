...
exports.getRegistrationPrice = async function() {
    ...
    const priceInWei = await contract.methods
        .getCurrentRegistrationPrice()
        .call();

    const priceInEth = web3Instance.utils.fromWei(
        priceInWei.toString(),
        "ether"
    );
...
function getContract() {
    ...
    const web3Instance = new web3(
        new HDWalletProvider(mnemonic, connectionString)
    );
...
function createGetBike(contract) {
    return async uniqueId => {
        const {
            ...
            state_,
            uniqueId_
        } = await contract.methods.getBicycle(uniqueId).call();
...
function createLookUpBike(contract) {
    return async (vendor, serialNumber, frameNumber) => {
        const {
            ...
            .lookUpBicycle(vendor, serialNumber, frameNumber)
            .call();
