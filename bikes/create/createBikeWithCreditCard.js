...
const creditCardService = require("../../modules/creditCardService");
const priceService = require("../../price/priceService");

exports.createBike = async function(
    vendor,
    serialNumber,
    frameNumber,
    ipfsImageHash,
    ...
    try {
        const { hasError, message } = checkInput(
            vendor,
            serialNumber,
            frameNumber,
            ipfsImageHash,
        ...
        if (hasError)
            return { hasError: true, message: "input error: " + message };

        const { priceInCents } = await priceService.getPrice();
        await creditCardService.charge(priceInCents, stripeToken);

        const transactionHash = await smartContractService.register(
            vendor,
            serialNumber,
            frameNumber,
            ipfsImageHash,
...
function checkInput(
    ...
    if (!serialNumber)
        return { hasError: true, message: "serialNumber missing" };
    if (!frameNumber) return { hasError: true, message: "frameNumber missing" };
    if (!ipfsImageHash)
        return { hasError: true, message: "ipfsImageHash missing" };
    if (!stripeToken) return { hasError: true, message: "stripeToken missing" };
    if (!ownerEthAddress)
        return { hasError: true, message: "ownerEthAddress missing" };
